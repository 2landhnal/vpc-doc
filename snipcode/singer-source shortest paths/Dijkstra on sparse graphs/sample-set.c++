...
// d[]: lưu khoảng cách ngắn nhất từ s đến mỗi đỉnh.

// p[]: lưu cha của mỗi đỉnh (dùng để phục hồi đường đi).

// set<pair<int,int>> q: hoạt động như một priority queue nhưng cho phép xóa phần tử bất kỳ (cần khi cập nhật khoảng cách).